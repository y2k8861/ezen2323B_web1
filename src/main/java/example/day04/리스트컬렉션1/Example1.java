...
public class Example1 {
    public static void main(String[] args) {
        ...
        // 컬렉션 프레임워크
            ...
            // 2. 널리 사용되는 자료구조(데이터를 저장/관리하는 방법론) 기반으로 제공한다.
            // 3. 대표 List, Set, Map 인터페이스

        // 1. List
            // - 객체를 저장하면 인덱스가 부여되고, 중복 가능
            // - ArrayList, Vector, LinkedList 클래스
                // 동기화 : 여러 스레드가 하나의 함수를 동시에 호출했을때 호출순서대로 해당 함수를 끝날떄까지 점유 상태
                // ArrayList : [배열구조] 동기화 지원 X, 주로 단일 스레드 사용
                // Vector : [배열구조] 동기화 지원 O, 주로 멀티스레드 사용
                // LinkedList : [연결구조] 동기화 지원,
                    // - 순차적 배열은 ArrayList 권장
                    // - 빈번한 중간 삽입과 삭제는 LinkedList 권장.
        ...
        // 제공하는 함수
            ...
            // 3. 리스트객체명.set(인덱스,객체)    : 주어진 객체를 리스트내 주어진 인덱스에 수정
            // 4. 리스트객체명.size()             : 리스트내 저장된 전체 객체 수를 반환
            // 5. 리스트객체명.get(인덱스)         : 주어진 인덱스에 저장된 객체 반환
            // 6. 리스트객체명.contains(객체)     : 주어진 객체가 리스트에 있는지 확인
            // 7. 리스트객체명.remove(인덱스)     : 주어진 인덱스에 저장된 객체 삭제
            // 8. 리스트객체명.remove(객체)       : 주어진 객체 삭제
            // 9. 리스트객체명.clear()           : 리스트내 모든 객체 삭제
            // 10. 리스트객체명.empty()           : 리스트가 비어 있는지 확인[T/F]반환
        // 순화
            // 1. for(int i = 0; i< 리스트명.size(); i++){실행문;}
            // 2. for(타입 반복변수명 : 리스트/배열){실행문}
            // 3. 리스트명.forEach(반복변수 -> 실행문)
        ...
        System.out.println("2번 인덱스(3번쨰 위치)에 있는 객체 : " + board1);
        
        // 5. 리스트내 저장된 특정 인덱스의 객체 존재 여부 확인
